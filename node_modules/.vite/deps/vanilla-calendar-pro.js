...
var __defNormalProp = (e, t, a) => t in e ? __defProp(e, t, { enumerable: true, configurable: true, writable: true, value: a }) : e[t] = a;
...
var __publicField = (e, t, a) => (__defNormalProp(e, "symbol" != typeof t ? t + "" : t, a), a);
...
var parseDates = (e) => e.reduce((e2, t) => (t.match(/^(\d{4}-\d{2}-\d{2})$/g) ? e2.push(t) : t.replace(/(\d{4}-\d{2}-\d{2}).*?(\d{4}-\d{2}-\d{2})/g, (t2, a, n) => {
...
var initRange = (e) => {
  ...
  e.rangeMin = s ? getDateString(e.date.today) : e.settings.range.disableAllDays ? getDateString(new Date(e.selectedYear, e.selectedMonth, 1)) : e.settings.range.min, e.rangeMax = e.settings.range.disableAllDays ? getDateString(new Date(e.selectedYear, e.selectedMonth, 1)) : e.settings.range.max, e.rangeDisabled = e.settings.range.disabled && !e.settings.range.disableAllDays ? parseDates(e.settings.range.disabled) : [], e.rangeDisabled.length > 1 && e.rangeDisabled.sort((e2, t2) => +new Date(e2) - +new Date(t2)), e.rangeEnabled = e.settings.range.enabled ? parseDates(e.settings.range.enabled) : [], (null == (t = e.rangeEnabled) ? void 0 : t[0]) && (null == (a = e.rangeDisabled) ? void 0 : a[0]) && (e.rangeDisabled = e.rangeDisabled.filter((t2) => !e.rangeEnabled.includes(t2))), e.rangeEnabled.length > 1 && e.rangeEnabled.sort((e2, t2) => +new Date(e2) - +new Date(t2)), (null == (n = e.rangeEnabled) ? void 0 : n[0]) && e.settings.range.disableAllDays && (e.rangeMin = e.rangeEnabled[0], e.rangeMax = e.rangeEnabled[e.rangeEnabled.length - 1]);
};
...
var setVisibilityArrows = ({ arrowPrev: e, arrowNext: t, isPrevHidden: a, isNextHidden: n }) => {
...
var createWeekNumber = (e, t, a, n, s) => {
...
var createWeekNumbers = (e, t, a, n, s) => {
...
var setDayModifier = (e, t, a, n, s, l, r) => {
  var i, d, c;
...
var createDays = (e) => {
  ...
  (e.settings.range.disableAllDays || e.settings.range.disableWeekday) && e.rangeDisabled[0] && (e.rangeDisabled = []), t.forEach((t2, s) => {
    const l = new Date(n);
...
var MultipleParser = (e, t) => t.replace(/<#Multiple>(.*?)<#\/Multiple>/g, (t2, a) => {
  let n = "";
...
var createYears = (e, t) => {
  ...
  for (let t2 = e.viewYear - 7; t2 < e.viewYear + 8; t2++) {
    const r = t2 < e.dateMin.getFullYear() + s || t2 > e.dateMax.getFullYear();
    n.append(createYearEl(e, l, a, r, t2));
  }
};
...
var getLocaleWeekday = (e, t) => {
  const a = new Date(0, 0, t).toLocaleString(e.settings.lang, { weekday: "short" });
  e.locale.weekday.push(capitalizeFirstLetter(a));
};
var getLocaleMonth = (e, t) => {
  const a = new Date(0, t).toLocaleString(e.settings.lang, { month: "long" });
...
var createMonths = (e, t) => {
  ...
  for (let t2 = 0; t2 < 12; t2++) {
    const a2 = e.locale.months[t2], n2 = t2 < e.dateMin.getMonth() + relationshipID(e) && r <= e.dateMin.getFullYear() || t2 > e.dateMax.getMonth() + relationshipID(e) && r >= e.dateMax.getFullYear() || t2 !== s && !d.includes(t2);
    i.append(createMonthEl(e, c, s, a2, n2, t2));
  }
};
...
var getInputElement = (e, t, a) => e.querySelector(`.${t}${a ? ` input[name="${a}"]` : ""}`);
var addMouseEvents = (e, t, a) => {
...
var InputTime = (e, t, a, n) => `<label class="${t}"><input type="text"name="${e}"maxlength="2"value="${a}"${n ? "disabled" : ""}></label>`;
var RangeTime = (e, t, a, n, s, l) => `<label class="${t}"><input type="range"name="${e}"min="${a}"max="${n}"step="${s}"value="${l}"></label>`;
var createTime = (e) => {
...
var getTheme = (e, t) => themes.find((a) => {
  var n;
...
var trackChangesThemeInSystemSettings = (e, t) => {
  const a = (t2) => setTheme(e.HTMLElement, t2.matches ? "dark" : "light");
  a(t), "system" !== e.settings.visibility.theme || haveListener.check() || (t.addEventListener("change", a), haveListener.set());
};
var trackChangesThemeInHTMLElement = (e, t, a) => {
...
var changeTheme = (e) => {
  ...
  ({ light: () => setTheme(e.HTMLElement, "light"), dark: () => setTheme(e.HTMLElement, "dark"), system: () => detectTheme(e, t) })[e.settings.visibility.theme]();
};
var create = (e) => {
  const t = { default: () => {
...
var handleClickMonthOrYear = (e, t, a, n) => {
...
var handleClick = (e) => {
  e.HTMLElement.addEventListener("click", (t) => {
    handleClickArrow(e, t), handleClickWeekNumber(e, t), handleClickDay(e, t), handleClickMonthOrYear(e, t, "month", { header: e.CSSClasses.month, item: e.CSSClasses.monthsMonth, column: e.CSSClasses.columnMonth }), handleClickMonthOrYear(e, t, "year", { header: e.CSSClasses.year, item: e.CSSClasses.yearsYear, column: e.CSSClasses.columnYear });
  });
};
var update = (e, { year: t, month: a, dates: n, holidays: s, time: l } = {}) => {
  var r;
  if (!e.isInit)
...
var setPositionCalendar = (e, t, a) => {
  const n = { left: 0, center: e.offsetWidth / 2 - t.offsetWidth / 2, right: e.offsetWidth - t.offsetWidth };
  let s = e.offsetHeight, l = n[a];
  for (let t2 = e; t2; t2 = t2.offsetParent)
    s += t2.offsetTop || 0, l += t2.offsetLeft || 0;
  Object.assign(t.style, { left: `${l}px`, top: `${s}px` });
};
var handleInput = (e) => {
  let t = true;
  e.HTMLInputElement = e.HTMLElement;
  const a = () => setPositionCalendar(e.HTMLInputElement, e.HTMLElement, e.settings.visibility.positionToInput), n = (t2) => {
    var s;
    e && t2.target !== e.HTMLInputElement && !(null == (s = e.HTMLElement) ? void 0 : s.contains(t2.target)) && (e.HTMLInputElement && e.HTMLElement && actionsInput(e).hide(), window.removeEventListener("resize", a), document.removeEventListener("click", n, { capture: true }));
  };
  e.HTMLInputElement.addEventListener("click", () => {
    t ? (() => {
      const a2 = document.createElement("div");
      a2.className = `${e.CSSClasses.calendar} ${e.CSSClasses.calendarToInput} ${e.CSSClasses.calendarHidden}`, e.HTMLElement = a2, document.body.append(e.HTMLElement), setPositionCalendar(e.HTMLInputElement, a2, e.settings.visibility.positionToInput), t = false, setTimeout(() => actionsInput(e).show(), 0), update(e), handleClick(e);
    })() : (setPositionCalendar(e.HTMLInputElement, e.HTMLElement, e.settings.visibility.positionToInput), actionsInput(e).show()), window.addEventListener("resize", a), document.addEventListener("click", n, { capture: true });
  });
};
var destroy = (e) => {
  var t, a, n, s;
...
var VanillaCalendar = class extends DefaultOptionsCalendar {
  constructor(e, t) {
    if (super(), __publicField(this, "init", () => {
      return (e2 = this).HTMLOriginalElement = e2.HTMLElement.cloneNode(true), e2.isInit = true, void (e2.input ? handleInput(e2) : (setVariables(e2), create(e2), handleClick(e2)));
      var e2;
